...
function showMsg() {
    const isEmpty = uname.value === '' || pass.value === ''|| cpass.value==='';
    const samepass = pass.value == cpass.value;
    
    btn.classList.toggle('no-shift', !isEmpty && samepass);
    if (isEmpty) {
        btn.disabled = true
        ...
        btn.disabled = true
        msg.style.color = 'rgb(218 49 49)';
        msg.innerText = 'Please fill the same password';
    }  
    else {
